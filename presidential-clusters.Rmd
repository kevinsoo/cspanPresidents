...
<!-- Here's a plot of the standard deviations and ranges of each president's attributes. The presidents with the greatest variation have been marked out; in addition to Nixon, other presidents with great imbalance in their attributes are Jackson, LBJ, and Reagan. -->

<!-- ```{r var, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.width=4.5, fig.height=4} -->
<!-- d <- potus -->
...
<!-- There's a moderate correlation between overall scores and the amount of variation in the attributes ($r$ = .51); presidents with low variation tend to score lower overall, implying that presidents who score higher overall do so because they're much stronger on only a subset of attributes. However, I wouldn't read too much into this with the sample size and number of attributes in our dataset. -->

### What Makes A President?

<!-- Are there certain attributes that tend to 'go together' across presidents? The following plot shows the correlations between pairs of attributes. All pairs are strongly and positively correlated, although there are some pairs that are less correlated. *Pursued equal justice for all* is the most isolated attribute (its column/row has the darkest hue, indicating its correlation with other attributes is lower). -->

<!-- ```{r attrCorr, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.width=5, fig.height=4} -->
...
From the structure in the attributes we uncovered with clustering, we can try to identify different types of presidents. The following heatmap displays each president's scores on every attribute, with the columns arranged by the attribute clusters and the rows arranged by how similar the presidents are to each other according to the pattern of their scores across all clusters.
...
It's a little difficult to see how the presidents are clustered from the heatmap, so I'll plot the dendrogram of presidents separately. While the previous dendrogram clusters attributes based on how they vary together across presidents, this dendrogram clusters presidents based on how they vary together across clusters of attributes. I've cut the branches of this dendrogram to group the nodes into 7 clusters of presidents.
...
# cspan
cspan <- read.csv("potus.df.csv") %>% filter(Number!=22)
